...
from torchbci.kilosort4 import CCG
...
def extract(ops, bfile, U, device=torch.device('cuda'), progress_bar=None, optimize_memory=False, deterministic_mode=0):
    ...
    if optimize_memory:
        st = torch.zeros((10**6, 3), dtype=torch.float64, device=device)
    else:
    ...
    try:
        for ibatch in prog:
            ...
            if optimize_memory:
                stt = stt.to(dtype=st.dtype)
                st[k:k+nsp,0] = (stt[:,0] - nt + t_shift) - nt//2 + ops['nt0min']
                st[k:k+nsp,1] = stt[:,1]
                st[k:k+nsp,2] = th_amps.to(dtype=st.dtype).squeeze(-1)
            else:
                stt = stt.double()
...
def align_U(U, ops, device=torch.device('cuda')):
    wPCA = ops['wPCA'].to(device=device)
    wTEMP = ops['wTEMP'].to(device=device, dtype=wPCA.dtype)
    U = U.to(device=device, dtype=wPCA.dtype)
    Uex = torch.einsum('xyz, zt -> xty', U, wPCA)
    X = Uex.reshape(-1, ops['Nchan']).T
    X = conv1d(X.unsqueeze(1), wTEMP.unsqueeze(1), padding=ops['nt']//2)
    ...
    Unew = torch.einsum('xty, zt -> xzy', Unew, wPCA)#.transpose(1,2).cpu()
    return Unew, imax


def postprocess_templates(Wall, ops, clu, st, tF, device=torch.device('cuda'), optimize_memory=False):
    ...
    Wall3, _, _, _, _ = merging_function(
        ...
        0.9, 'mu', check_dt=False, device=device, optimize_memory=optimize_memory
        )
...
def prepare_matching(ops, U):
    nt = ops['nt']
    W = ops['wPCA'].to(device=U.device, dtype=U.dtype).contiguous()
...
def merging_function(ops, Wall, clu, st, tF, r_thresh=0.5, mode='ccg', check_dt=True,
                     device=torch.device('cuda'), optimize_memory=False):
    clu2 = clu.copy()
    clu_unq, ns = np.unique(clu2, return_counts = True)
    st_is_torch = torch.is_tensor(st)
    st_np = st.detach().cpu().numpy() if st_is_torch else st
    ...
    if mode == 'ccg':
        is_ref, est_contam_rate = CCG.refract(clu, st_np[:,0]/ops['fs'],
        ...
        for j in range(NN):
            ...
            if is_ccg:
                ...
                if dt != 0 and check_dt:
                    ...
                    idx = (clu2 == jj)
                    if optimize_memory:
                        idx_t = torch.from_numpy(idx).to(tF.device)
                        # Update tF and Wall with shifted features
                        tF, Wall = roll_features(W, tF, Ww, idx_t, jj, dt, optimize_memory=True)
                    else:
                        tF, Wall = roll_features(W, tF, Ww, idx, jj, dt, optimize_memory=False)
                    # Shift spike times
                    if st_is_torch:
                        idx_t = torch.from_numpy(idx).to(st.device)
                        st[idx_t,0] -= dt
                        st_np[idx,0] -= dt
                    else:
                        st[idx,0] -= dt
    ...
    if optimize_memory:
        keep_mask = torch.from_numpy(~is_merged).to(Ww.device)
        Ww = Ww[keep_mask]
    else:
        Ww = Ww[~is_merged]
    ...
    sorted_idx = np.argsort(st_np[:,0])
    if st_is_torch:
        sorted_idx_t = torch.from_numpy(sorted_idx).to(st.device)
        st = st[sorted_idx_t]
    else:
        st = np.take_along_axis(st, sorted_idx[..., np.newaxis], axis=0)
    clu2 = clu2[sorted_idx]
    if optimize_memory:
        tensor_idx = torch.from_numpy(sorted_idx).to(tF.device)
        tF = tF[tensor_idx]
        return Ww, clu2, is_ref, st, tF
    else:
...
def roll_features(wPCA, tF, Wall, spike_idx, clust_idx, dt, optimize_memory=False):
    compute_device = tF.device
    W = wPCA.to(device=compute_device, dtype=tF.dtype)
    wall_dtype = Wall.dtype
    Wall_t = Wall.to(device=compute_device, dtype=W.dtype)
    # Project from PC space back to sample time, shift by dt
    feats = torch.roll(tF[spike_idx] @ W, shifts=dt, dims=2)
    temps = torch.roll(Wall_t[clust_idx:clust_idx+1] @ W, shifts=dt, dims=2)
    ...
    Wall[clust_idx] = (temps @ W.T).to(device=Wall.device, dtype=wall_dtype)
    return tF, Wall
