...
from torchbci.kilosort4.preprocessing import get_drift_matrix, fft_highpass
from torchbci.kilosort4.postprocessing import (
...
def load_probe(probe_path):
    ...
    if probe_path.suffix == '.prb':
        ...
        probe['xc_t'] = torch.as_tensor(probe['xc'])
        probe['yc_t'] = torch.as_tensor(probe['yc'])
...
def save_to_phy(st, clu, tF, Wall, probe, ops, imin, results_dir=None,
                ...
                save_preprocessed_copy=False, skip_dat_path=False, optimize_memory=False):
    ...
    if torch.is_tensor(st):
        ...
        st_cpu = st
    spike_times = st_cpu[:,0].astype('int64') + imin  # shift by minimum sample index
    spike_templates = st_cpu[:,1].astype('int32')
    spike_clusters = clu
    xs, ys = compute_spike_positions(st, tF, ops, optimize_memory=optimize_memory)
    ...
    if optimize_memory:
        kept_spikes_t = torch.from_numpy(kept_spikes).to(tF.device)
        tF = tF[kept_spikes_t]
    else:
        tF = tF[kept_spikes]
    pc_features, pc_feature_ind = make_pc_features(
        ops, spike_templates, spike_clusters, tF, optimize_memory=optimize_memory
        )
    if optimize_memory:
        np.save(results_dir / 'pc_features.npy', pc_features.cpu().numpy())
    else:
    ...
    if save_extra_vars:
        ...
        np.save(results_dir / 'full_st.npy', st_cpu)
...
def save_ops(ops, results_dir=None):
    ...
    ops['settings']['data_dir'] = str(['data_dir'])
    ...
    def filter(self, X, ops=None, ibatch=None, skip_preproc=False):
        ...
        if self.do_CAR:
            ...
            if self.deterministic_mode == 2:
                X = X - torch.median(X.cpu(), 0)[0].to(X.device)
            else:
                X = X - torch.median(X, 0)[0]
        if skip_preproc:
            return X
        
        # Custom filtering function (Jim's)
