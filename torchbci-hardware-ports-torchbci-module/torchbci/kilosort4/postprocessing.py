...
from torchbci.kilosort4.clustering_qr import xy_templates, get_data_cpu
...
def compute_spike_positions(st, tF, ops, optimize_memory=False):
    ...
    if optimize_memory:
        device = tF.device
        icc_mask = ops['iCC_mask'].to(device)
        iU = ops['iU'].to(device)
        iCC = ops['iCC'].to(device)
        spike_templates = torch.as_tensor(st[:, 1], device=device, dtype=torch.long)
    else:
    ...
    if optimize_memory:
        tmask = icc_mask[:, iU[spike_templates]].T
    else:
...
def make_pc_features(ops, spike_templates, spike_clusters, tF, optimize_memory=False):
    ...
    for i in np.unique(spike_clusters):
        ...
        if optimize_memory:
            ix = torch.zeros(n_templates, dtype=torch.bool, device=device)
            ix[torch.as_tensor(iunq, device=device)] = True
        else:
        ...
        Xd, igood, ichan = get_data_cpu(
            ...
            dmin=ops['dmin'], dminx=ops['dminx'], ix=ix, merge_dim=False, optimize_memory=optimize_memory
            )
