...
def log_sorting_summary(ops, log=None, level=None):
    ...
    mean_drift_str = _format_mean_drift(ops)
    log_fn(f"{'Mean abs. drift':<30}{mean_drift_str:>25}")
    ...
    usage_postproc = ops.get('usage_postproc')
    gpu = bool(
        usage_postproc
        and isinstance(usage_postproc, dict)
        and usage_postproc.get('gpu') is not None
    )
    log_fn(' ')
...
def _format_runtime(ops, name, key):
    total = ops['runtime']
    step = ops.get('runtime_' + key)
    if total in (None, 0) or step is None:
        step_str = f"{'N/A':>16}"
        pct_str = '(N/A) %'
    else:
        pct = step / total * 100
        step_str = f"{step:>15.1f}s"
        pct_str = f"({pct:.2f}) %"
    return f"{(name + ':'):<27} {step_str} {pct_str:>10}"

def _format_usage(ops, name, key, gpu):
    usage = ops.get('usage_' + key)
    if not usage or usage.get('cpu') is None:
        sys_part = f"sys  {'N/A':>6} GB  |  "
        if gpu:
            gpu_part = f"gpu  {'N/A':>5} GB"
        else:
            gpu_part = f"gpu  {'N/A':>7}   "
        return f"{(name + ':'):<23}{sys_part}{gpu_part}"
    s = (f"{(name + ':'):<23}"
         + f"sys  {usage['cpu']['mem_used']:>6.1f} GB  |  ")
    if gpu and usage.get('gpu') is not None:
        s += f"gpu  {usage['gpu']['max_alloc']:>5.1f} GB"
    else:
        s += f"gpu  {'N/A':>7}   "
    return s

def _format_mean_drift(ops):
    mean_drift = ops.get('mean_drift')
    if mean_drift is None:
        return 'N/A'
    try:
        if np.isnan(mean_drift):
            return 'N/A'
    except TypeError:
        return 'N/A'
    return f'{mean_drift:.1f} um'
