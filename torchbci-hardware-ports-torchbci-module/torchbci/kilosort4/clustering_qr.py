...
def get_faiss_res(temp_mb=256):
    global _FAISS_RES
    if _FAISS_RES is None:
        _FAISS_RES = faiss.StandardGpuResources()
        # Reduce scratch to limit peak allocations
        _FAISS_RES.setTempMemory(temp_mb * 1024 * 1024)
    return _FAISS_RES

from tqdm import tqdm 

from torchbci.kilosort4 import hierarchical, swarmsplitter
from torchbci.kilosort4.utils import log_performance

logger = logging.getLogger(__name__)


def neigh_mat(Xd, nskip=1, n_neigh=10, max_sub=25000,
              optimize_memory=False, device=torch.device('cpu')):
    ...
    if use_faiss_gpu:
        try:
            res = get_faiss_res(temp_mb=256)
            index = faiss.IndexFlatL2(dim)
            index = faiss.index_cpu_to_gpu(res, 0, index)
            # print("Using FAISS GPU index for neighbor search")
        except Exception:
            # print("FAISS GPU path unavailable, falling back to CPU.")
            use_faiss_gpu = False
            index = faiss.IndexFlatL2(dim)
    else:
        # print("FAISS CPU")
    ...
    Xsub = np.ascontiguousarray(Xsub, dtype=np.float32)
    index.add(Xsub)    # add vectors to the index
    _, kn = index.search(Xd, n_neigh)     # actual search
    if torch.is_tensor(kn):
        kn = kn.detach().cpu().numpy()
...
def cluster(Xd, iclust=None, kn=None, nskip=1, n_neigh=10, max_sub=25000,
            ...
            verbose=False, optimize_memory=False):    
    if kn is None:
        ...
        if torch.is_tensor(Xd):
            Xd_np = Xd.detach().cpu().numpy()
        else:
            Xd_np = Xd
        kn, M = neigh_mat(Xd_np, nskip=nskip, n_neigh=n_neigh, max_sub=max_sub, optimize_memory=optimize_memory, device=device)
...
def kmeans_plusplus(Xg, niter=200, seed=1, device=torch.device('cuda'), verbose=False):
    ...
    mu = torch.zeros((niter, n_features), device=device, dtype=dtype)
    vexp0 = torch.zeros(n_spikes, device=device, dtype=dtype)
...
def xy_up(ops):
    ...
    if torch.is_tensor(xcup) or torch.is_tensor(ycup):
        if not torch.is_tensor(xcup):
            xcup = torch.as_tensor(xcup, device=ycup.device)
        if not torch.is_tensor(ycup):
            ycup = torch.as_tensor(ycup, device=xcup.device)
        if xcup.device != ycup.device:
            ycup = ycup.to(xcup.device)
        xy = torch.vstack((xcup, ycup))
    else:
...
def y_centers(ops):
    ...
    if torch.is_tensor(ycup):
        start = float(ycup.min().item() + dmin - 1)
        end = float(ycup.max().item() + dmin + 1)
...
def get_nearest_centers(xy, xcent, ycent):
    if torch.is_tensor(xy):
        ...
        if torch.is_tensor(xcent):
            device = xcent.device
            dtype = xcent.dtype
        elif torch.is_tensor(ycent):
            device = ycent.device
            dtype = ycent.dtype
        else:
            device = None
            dtype = torch.float32
        xy_t = torch.as_tensor(xy, device=device, dtype=dtype)
    xcent_t = torch.as_tensor(xcent, device=xy_t.device, dtype=xy_t.dtype)
    ycent_t = torch.as_tensor(ycent, device=xy_t.device, dtype=xy_t.dtype)
    # Get positions of all grouping centers
    ycent_pos, xcent_pos = torch.meshgrid(ycent_t, xcent_t, indexing='xy')
    ycent_pos = ycent_pos.flatten()
    xcent_pos = xcent_pos.flatten()
    # Compute distances from templates
    center_distance = (
        (xy_t[0,:] - xcent_pos.unsqueeze(-1))**2
        + (xy_t[1,:] - ycent_pos.unsqueeze(-1))**2
        )
    # Add some randomness in case of ties
    center_distance += 1e-20 * torch.rand(
...
def run(ops, st, tF, mode='template', device=torch.device('cuda'),
        progress_bar=None, clear_cache=False, verbose=False, optimize_memory=False):

    if mode == 'template':
        xy, iC = xy_templates(ops)
        if optimize_memory and torch.is_tensor(st):
            iclust_template = st[:,1].to(dtype=torch.long)
        else:
        ...
        xy, iC = xy_up(ops)
        if optimize_memory and torch.is_tensor(st):
            iclust_template = st[:,5].to(dtype=torch.long)
        else:
    ...
    if torch.is_tensor(nearest_center):
        center_set = set(nearest_center.detach().cpu().tolist())
    else:
        center_set = set(np.asarray(nearest_center).tolist())
    total_centers = len(center_set)
    ycent_size = ycent.numel() if torch.is_tensor(ycent) else np.size(ycent)
    xcent_size = xcent.numel() if torch.is_tensor(xcent) else np.size(xcent)
    
    clu = np.zeros(nsp, 'int32')
    if optimize_memory:
        Wall = torch.zeros(
            (0, ops['Nchan'], ops['settings']['n_pcs']),
            device=device,
        )
    else:
    ...
    nmax = 0
    prog = tqdm(np.arange(xcent_size), miniters=20 if progress_bar else None,
        ...
        for jj in prog:
            for kk in range(ycent_size):
                ...
                Xd, igood, ichan = get_data_cpu(
                    ...
                    dmin=dmin, dminx=dminx, ix=ix, optimize_memory=optimize_memory
                    )
                if Xd is None:
                ...
                if Xd.shape[0] < 1000:
                    ...
                    if mode == 'template':
                        if optimize_memory:
                            if torch.is_tensor(st):
                                st0 = st[igood,0].detach().cpu().numpy() / ops['fs']
                            else:
                                st0 = st[igood_np,0]/ops['fs']
                        else:
                    ...
                    iclust, iclust0, M, _ = cluster(
                        ...
                        lam=1, seed=seed, device=device, verbose=v, optimize_memory=optimize_memory
                        )
                    ...
                    if optimize_memory:
                        Xd_cpu = Xd.detach().cpu().numpy()
                        xtree, tstat = swarmsplitter.split(
                            Xd_cpu, xtree, tstat, iclust, my_clus, meta=st0
                            )
                    else:
                        xtree, tstat = swarmsplitter.split(
                ...
                if optimize_memory:      
                    W = torch.zeros(
                        (Nfilt, ops['Nchan'], ops['settings']['n_pcs']),
                        device=device,
                        dtype=ops['wPCA'].dtype,
                    )
                    iclust_t = torch.as_tensor(iclust, device=Xd.device)
                    for j in range(Nfilt):
                        w = Xd[iclust_t==j].mean(0)
                        w = w.to(dtype=W.dtype)
                        W[j, ichan, :] = torch.reshape(w, (-1, ops['settings']['n_pcs']))
                else:
                    W = torch.zeros(
                        (Nfilt, ops['Nchan'], ops['settings']['n_pcs']),
                        dtype=ops['wPCA'].dtype,
...
def get_data_cpu(ops, xy, iC, PID, tF, ycenter, xcenter, dmin=20, dminx=32,
                 ix=None, merge_dim=True, optimize_memory=False):
    if optimize_memory:
        device = tF.device
        PID_t = torch.as_tensor(PID, device=device, dtype=torch.long)
    else:
    ...
    if optimize_memory:
        ...
        elif ix.device != device:
            ix = ix.to(device)
        igood = ix[PID_t].nonzero()[:, 0]
    else:
    ...
    if optimize_memory:
        if not torch.is_tensor(iC):
            iC = torch.as_tensor(iC, device=device)
        elif iC.device != device:
            iC = iC.to(device)
    ichan, imap = torch.unique(iC[:, ix], return_inverse=True)
    if optimize_memory:
        if ichan.device != device:
            ichan = ichan.to(device)
        if imap.device != device:
            imap = imap.to(device)
    nchan = ichan.nelement()

    if optimize_memory:
        dd = torch.zeros(
            (nspikes, nchan, nfeatures),
            device=device,
            dtype=data.dtype,
        )
    else:
