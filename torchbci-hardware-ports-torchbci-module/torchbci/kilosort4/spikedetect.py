...
# cuML KMeans determinism is enforced where supported.
_HAS_CUML = False
try:
    import cupy as cp
...
from torchbci.kilosort4.utils import template_path, log_performance
...
def extract_wPCA_wTEMP(ops, bfile, nt=61, twav_min=20, Th_single_ch=6, nskip=25,
                       device=torch.device('cuda'), optimize_memory=False, deterministic_mode=0, seed=1):
    if optimize_memory:
        clips = torch.zeros((500000, nt), device=device, dtype=torch.float32)
    else:
    ...
    clips = clips[:i]
    if optimize_memory:
        clips /= (clips**2).sum(1, keepdim=True).sqrt()
    else:
        clips /= (clips**2).sum(1, keepdims=True)**.5
    if optimize_memory and deterministic_mode == 0:
        # KMEANS CuML VERSION
        _, _, Vh = torch.linalg.svd(clips, full_matrices=False)
        n_pcs = ops['settings']['n_pcs']
        wPCA = Vh[:n_pcs].contiguous()
        if _HAS_CUML and device.type == "cuda":
            # Move Torch -> CuPy without a copy via DLPack
            clips_cp = cp.from_dlpack(torch.utils.dlpack.to_dlpack(clips.contiguous()))
            # Deterministic init: first k points (works if your input ordering is stable)
            # init_centers = clips_cp[:ops["settings"]["n_templates"]].copy()
            # cuML KMeans runs on GPU
            # Note: cuML defaults/args can vary slightly by version; these are commonly supported.
            cp.random.seed(seed)
            km = cuKMeans(
                n_clusters=ops['settings']['n_templates'],
            ...
            centers_cp = cp.asarray(km.cluster_centers_)  # CuPy array on GPU

            # CuPy -> Torch (GPU) via DLPack
            wTEMP = torch.utils.dlpack.from_dlpack(centers_cp).to(device=device, dtype=torch.float32)
            wTEMP = wTEMP / (wTEMP**2).sum(1, keepdim=True).sqrt()
            ...
            with warnings.catch_warnings():
                ...
                model = KMeans(n_clusters=ops['settings']['n_templates'], n_init = 10, random_state=seed).fit(clips)
        ...
        if torch.is_tensor(clips):
            clips = clips.contiguous().cpu().numpy()
        model = TruncatedSVD(n_components=ops['settings']['n_pcs'], random_state=seed).fit(clips)
...
def nearest_chans(ys, yc, xs, xc, nC, device=torch.device('cuda'), optimize_memory=False):
    if optimize_memory:
        if not torch.is_tensor(ys):
            ys = torch.as_tensor(ys, device=device, dtype=torch.float32)
            xs = torch.as_tensor(xs, device=device, dtype=torch.float32)
            yc = torch.as_tensor(yc, device=device, dtype=torch.float32)
            xc = torch.as_tensor(xc, device=device, dtype=torch.float32)
        ys_ = ys.unsqueeze(0)          # (1, N)
        xs_ = xs.unsqueeze(0)          # (1, N)
        yc_ = yc.unsqueeze(1)          # (M, 1)
        xc_ = xc.unsqueeze(1)          # (M, 1)

        ds_all = (ys_ - yc_)**2 + (xs_ - xc_)**2   # (M, N)
        ds, iC = torch.topk(ds_all, k=nC, dim=0, largest=False, sorted=True)
    else:
...
def run(ops, bfile, device=torch.device('cuda'), progress_bar=None,
        clear_cache=False, verbose=False, optimize_memory=False, deterministic_mode=0, seed=1):        
    ...
    if ops['settings']['templates_from_data']:
        ...
        ops['wPCA'], ops['wTEMP'] = extract_wPCA_wTEMP(
            ...
            device=device, optimize_memory=optimize_memory, deterministic_mode=deterministic_mode, seed=seed
            )
    else:
    ...
    if optimize_memory:
        xc = ops['xc_t'].to(device)
        yc = ops['yc_t'].to(device)
        [ys_t, xs_t] = torch.meshgrid(
            torch.as_tensor(ops['yup'], device=device, dtype=torch.float32),
            torch.as_tensor(ops['xup'], device=device, dtype=torch.float32),
            indexing='xy',
        )
        ys, xs = ys_t.flatten(), xs_t.flatten()
    else:
    ...
    iC, ds = nearest_chans(ys, yc, xs, xc, nC, device=device, optimize_memory=optimize_memory)
    ...
    iC2, _ = nearest_chans(ys, ys, xs, xs, nC2, device=device, optimize_memory=optimize_memory)
    if optimize_memory:
        ds_torch = ds
    else:
    ...
    if optimize_memory:
        st = torch.zeros((10**6, 6), dtype=torch.float64, device=device)
        if deterministic_mode == 1:
            tF = torch.zeros(
                (10**6, nC, ops['settings']['n_pcs']),
                dtype=torch.float64,
                device=device,
            )
        else:
        ...
        if deterministic_mode == 1:
            tF = np.zeros((10**6, nC , ops['settings']['n_pcs']), 'float64')
        else:
    ...
    try:
        for ibatch in prog:
            ...
            if optimize_memory:
                tF[k:k+nsp] = xfeat.transpose(0,1)
            else:
                tF[k:k+nsp] = xfeat.transpose(0,1).cpu().numpy()

            t_shift = ibatch * bfile.batch_downsampling * (ops['batch_size']/ops['fs'])
            if optimize_memory:
                st[k:k+nsp,0] = (xy[:,1].to(st.dtype) - nt) / ops['fs'] + t_shift
                st[k:k+nsp,1] = yct.to(st.dtype)
                st[k:k+nsp,2] = amp.to(st.dtype)
                st[k:k+nsp,3] = imax.to(st.dtype)
                st[k:k+nsp,4] = float(ibatch)
                st[k:k+nsp,5] = xy[:,0].to(st.dtype)
            else:
