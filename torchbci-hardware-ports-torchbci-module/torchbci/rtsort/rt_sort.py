...
from torchbci.rtsort.model import ModelSpikeSorter
...
def detect_sequences(
    ...
    Args:
        ...
        optimize_memory (bool, optional):
            Whether to use lower-peak-memory chunked cluster setup statistics. Defaults to False.
    ...
    with warnings.catch_warnings():
        ...
        if detection_model is None:
            ...
            # print("loading in 32 if deterministic")
            detection_model = ModelSpikeSorter.load(detection_model)
            # detection_model.dtype = torch.float32 if deterministic_mode == 2 else torch.float16    
            # print(f"Using detection model with dtype {detection_model.dtype} for deterministic_mode {deterministic_mode}")    
        # Set up paths
        ...
        if debug and verbose and (inter_path / "scaled_traces.npy").exists():
            ...
            save_traces(recording, inter_path, *recording_window_ms, num_processes=num_processes_save_traces, verbose=verbose,
                        dtype="float16", optimize_memory=optimize_memory)
        ...
        if debug and verbose and (model_inter_path / "model_outputs.npy").exists():
            ...
            run_detection_model(recording,
                                ...
                                model_inter_path,
                                device, verbose, optimize_memory=optimize_memory, deterministic_mode=deterministic_mode)
...
def _ensure_reusable_torch_buffer(buffer, shape, device, dtype):
...
def _copy_array_to_reusable_torch_buffer(
    array,
    *,
    device,
    dtype,
    ...
    if transpose:
        np_array = np.ascontiguousarray(np_array.T)
    else:
        np_array = np.ascontiguousarray(np_array)
    if not np_array.flags.writeable:
        # np.load(..., mmap_mode="r") returns read-only views. Copy once so
        # torch.as_tensor does not point at non-writable memory.
        np_array = np_array.copy()

    source = torch.as_tensor(np_array, dtype=dtype)
    buffer = _ensure_reusable_torch_buffer(
        buffer,
        source.shape,
        device=device,
        dtype=dtype,
...
class RTSort:
    def __init__(self, sequences, model, params: dict,
        ...
        self.model_inter_path = Path(model_inter_path)
        ...
        self.device = device
        self.dtype = dtype
        self.optimize_memory = params.get("optimize_memory", False)
        self.deterministic_mode = params.get("deterministic_mode", 0)
        # Coarse score quantization makes strict-mode assignment resilient to
        # tiny backend numeric jitter while preserving ordering at larger gaps.
        self.deterministic_score_quant = 128.0
        self.deterministic_score_tie_bins = 1
        self.deterministic_logit_quant = 128.0

        # if self.deterministic_mode == 0:
        #     torch.backends.cudnn.benchmark = True

        deterministic_strict = self.deterministic_mode == 2
        if deterministic_strict:
            # For strict determinism, avoid TensorRT/TorchScript compilation and run
            # a single PyTorch inference path for every chunk.
            self.model = model.model.conv.to(device=device, dtype=dtype)
        elif device == "cuda":
            compiled_model_path = model_inter_path / ModelSpikeSorter.compiled_name
            # Only reuse a cached compiled model when running in float16; other dtypes
            # (float32, float64) always recompile to avoid loading a stale float16 binary.
            use_cached = (
                compiled_model_path.exists()
                and ModelSpikeSorter.can_load_compiled()
                and dtype == torch.float16
            )
            if use_cached:
                try:
                    self.model = ModelSpikeSorter.load_compiled(model_inter_path, device=device)
                except Exception as exc:
                    print(f"Skipping cached TensorRT model at {compiled_model_path} because it could not be loaded in the current runtime: {exc}")
                    self.model = model.compile(len(elec_locs), None, dtype=dtype)
            else:
                if compiled_model_path.exists() and not ModelSpikeSorter.can_load_compiled():
                    print("Skipping cached TensorRT model because TorchScript module loading is unavailable in the current runtime. This can happen under profilers such as Scalene.")
                self.model = model.compile(len(elec_locs), None, dtype=dtype)
        else:
            self.model = model.model.conv.to(device=device, dtype=dtype)
        self.model.eval()
        ...
        self._runtime_obs_buffer = None
        self._runtime_model_chunk_buffer = None
    ...
    def to_dtype(self, dtype):
        """
        Convert floating-point runtime tensors to :dtype:.
        """
        if self.dtype == dtype:
            return self
        ...
        for attr in float_tensor_attrs:
            value = getattr(self, attr, None)
            if isinstance(value, torch.Tensor) and value.is_floating_point():
                setattr(self, attr, value.to(dtype=dtype))

        if isinstance(self.pre_medians, torch.Tensor) and self.pre_medians.is_floating_point():
            self.pre_medians = self.pre_medians.to(dtype=dtype)
    ...
    def to_device(self, device):
        """
        Move runtime tensors to :device:.
        """
        device = torch.device(device)
        if torch.device(self.device) == device:
            return self
        ...
        if isinstance(self.pre_medians, torch.Tensor):
            self.pre_medians = self.pre_medians.to(device=device)
    ...
    def running_sort(self, obs, model_chunk=None, latest_frame=None):
        ...
        # if getattr(self, "deterministic_mode", 0) == 2:
        ...
        if self.optimize_memory:
            obs = _copy_array_to_reusable_torch_buffer(
                obs,
                device=self.device,
                dtype=self.dtype,
                buffer=self._runtime_obs_buffer,
                transpose=True,
            )
            self._runtime_obs_buffer = obs
        else:
            obs = torch.tensor(obs, device=self.device, dtype=self.dtype).T
        if model_chunk is not None:
            if self.optimize_memory:
                model_chunk = _copy_array_to_reusable_torch_buffer(
                    model_chunk,
                    device=self.device,
                    dtype=self.dtype,
                    buffer=self._runtime_model_chunk_buffer,
                )
                self._runtime_model_chunk_buffer = model_chunk
            else:
                model_chunk = torch.tensor(model_chunk, device=self.device, dtype=self.dtype)
        else:
            obs -= torch.median(obs.cpu(), dim=1, keepdim=True).values.to(self.device)
        num_new_frames = int(obs.shape[1])
        if num_new_frames <= 0:
            return []

        # Update internal traces cache
        cache_frames = int(self.pre_median_frames.shape[1])
        if num_new_frames >= cache_frames:
            self.pre_median_frames.copy_(obs[:, -cache_frames:])
        else:
            if self.deterministic_mode == 2 or self.deterministic_mode == 1:
                self.pre_median_frames[:, :-num_new_frames] = self.pre_median_frames[:, num_new_frames:].clone()
            else:
    ...
    def sort_offline(self, recording,
        ...
        model_chunk = None
        deterministic_cached_outputs = (
            model_outputs is not None and getattr(self, "deterministic_mode", 0) == 2
        )
        padded_model_chunk = None
        for start_frame in all_start_frames:
            if model_outputs is not None:
                ...
                elif deterministic_cached_outputs:
                    if padded_model_chunk is None:
                        padded_model_chunk = np.full(
                            (model_outputs.shape[0], self.model_num_output_locs),
                            -20,
                            dtype=model_outputs.dtype,
                        )
                    model_chunk = padded_model_chunk
                else:
                    model_chunk = None
    ...
    def _select_deterministic_spike_candidate(self, spike_scores, peak_ind_flat):
        """
        Select the next spike candidate with a stable tie-break order.

        Primary key: highest score.
        Secondary key: earliest peak index.
        Tertiary key: lowest sequence index.
        """
        if getattr(self, "deterministic_mode", 0) == 2:
            positive_mask = spike_scores > 0
            positive_peaks = torch.nonzero(torch.any(positive_mask, dim=1), as_tuple=True)[0]
            if positive_peaks.numel() == 0:
                return None

            spike_idx = positive_peaks[0]
            row_scores = spike_scores[spike_idx]
            quant = float(getattr(self, "deterministic_score_quant", 128.0))
            tie_bins = int(getattr(self, "deterministic_score_tie_bins", 1))
            row_scores_q = torch.round(row_scores * quant).to(torch.int32)
            row_max = torch.max(row_scores_q)
            if not torch.gt(row_max, 0):
                return None
            seq_candidates = torch.nonzero(
                row_scores_q >= (row_max - tie_bins), as_tuple=True
            )[0]
            seq_idx = seq_candidates[0]
            best_flat_idx = spike_idx * self.num_seqs + seq_idx
            return best_flat_idx, spike_idx, seq_idx

        flat_scores = spike_scores.reshape(-1)
        max_score = torch.max(flat_scores)
        if not torch.gt(max_score, 0):
            return None

        # In strict deterministic mode, spike_scores are quantized so exact equality
        # is preferable to tolerance-based matching for tie handling.
        candidate_mask = flat_scores == max_score
        if not torch.any(candidate_mask):
            score_dtype = spike_scores.dtype if spike_scores.is_floating_point() else torch.float32
            atol = torch.finfo(score_dtype).eps * 8
            candidate_mask = torch.isclose(flat_scores, max_score, rtol=0.0, atol=atol)
        candidate_indices = torch.nonzero(candidate_mask, as_tuple=True)[0]
        if candidate_indices.numel() == 1:
            best_flat_idx = candidate_indices[0]
        else:
            candidate_spike_idx = torch.div(candidate_indices, self.num_seqs, rounding_mode='floor')
            candidate_seq_idx = torch.remainder(candidate_indices, self.num_seqs)
            tie_break_keys = peak_ind_flat[candidate_spike_idx].to(torch.int64) * self.num_seqs + candidate_seq_idx.to(torch.int64)
            best_flat_idx = candidate_indices[torch.argmin(tie_break_keys)]

        spike_idx = torch.div(best_flat_idx, self.num_seqs, rounding_mode='floor')
        seq_idx = torch.remainder(best_flat_idx, self.num_seqs)
        return best_flat_idx, spike_idx, seq_idx
    ...
    def sort_chunk(self, chunk, torch_window=None,
        ...
        deterministic_strict = getattr(self, "deterministic_mode", 0) == 2
        score_dtype = torch.float32 if deterministic_strict else self.dtype
        comp_elecs = self.comp_elecs
        spike_arange = self.spike_arange
        seqs_inner_loose_elecs = self.seqs_inner_loose_elecs
        seqs_loose_elecs = self.seqs_loose_elecs
        seqs_min_loose_elecs = self.seqs_min_loose_elecs
        seqs_latencies = self.seqs_latencies
        seqs_latency_weights = self.seqs_latency_weights
        seqs_amps = self.seqs_amps
        seqs_amp_weights = self.seqs_amp_weights
        seqs_root_amp_means = self.seqs_root_amp_means
        seqs_root_amp_stds = self.seqs_root_amp_stds
        seq_no_overlap_mask = self.seq_no_overlap_mask
        ...
        if deterministic_strict:
            logit_quant = float(getattr(self, "deterministic_logit_quant", 128.0))
            if torch_window.dtype != torch.float32:
                torch_window = torch_window.to(torch.float32)
            torch_window = torch.round(torch_window * logit_quant) / logit_quant

        stringent_thresh_logit = self.stringent_thresh_logit
        loose_thresh_logit = self.loose_thresh_logit
        if deterministic_strict:
            logit_quant = float(getattr(self, "deterministic_logit_quant", 128.0))
            stringent_thresh_logit = round(float(stringent_thresh_logit) * logit_quant) / logit_quant
            loose_thresh_logit = round(float(loose_thresh_logit) * logit_quant) / logit_quant
        ...
        peak_ind_flat = peak_ind_flat[torch.sum(torch_window[:, peak_ind_flat+self.seq_n_before]
                                                >= loose_thresh_logit, dim=0) <= self.min_elecs_for_array_noise]  # may be slow
        ...
        # start = perf_counter()
        latencies_float = latencies.transpose(1, 2).to(score_dtype)
        seqs_latencies = seqs_latencies if seqs_latencies.dtype == score_dtype else seqs_latencies.to(score_dtype)
        seqs_latency_weights = seqs_latency_weights if seqs_latency_weights.dtype == score_dtype else seqs_latency_weights.to(score_dtype)
        ...
        rec_window_scores = rec_window if rec_window.dtype == score_dtype else rec_window.to(score_dtype)
        pre_medians_scores = pre_medians if pre_medians.dtype == score_dtype else pre_medians.to(score_dtype)
        seqs_root_amp_means = seqs_root_amp_means if seqs_root_amp_means.dtype == score_dtype else seqs_root_amp_means.to(score_dtype)
        seqs_root_amp_stds = seqs_root_amp_stds if seqs_root_amp_stds.dtype == score_dtype else seqs_root_amp_stds.to(score_dtype)
        seqs_amps = seqs_amps if seqs_amps.dtype == score_dtype else seqs_amps.to(score_dtype)
        seqs_amp_weights = seqs_amp_weights if seqs_amp_weights.dtype == score_dtype else seqs_amp_weights.to(score_dtype)

        amps = torch.abs(rec_window_scores[comp_elecs, peak_ind + latencies].transpose(1, 2)) / pre_medians_scores

        root_amp_z = torch.abs(amps[:, 0, self.seqs_root_elecs_rel_comp_elecs] -
                               seqs_root_amp_means) / seqs_root_amp_stds
        ...
        strict_crosses_root = spike_window[:, self.seqs_root_elecs_rel_comp_elecs,
                                           self.seq_n_before] >= stringent_thresh_logit
        ...
        # start = perf_counter()
        total_loose_crosses = torch.sum(elec_crosses, dim=2).to(score_dtype).clamp_min(1)
        spike_scores = latency_diff / self.max_latency_diff_spikes + amp_diff / self.max_amp_median_diff_spikes - (num_loose.to(score_dtype) / total_loose_crosses * 0.5)
        spike_scores = 2.1 - spike_scores  # (additional 0.1 in case spike_scores=2)
        spike_scores *= can_spike
        if deterministic_strict:
            # Collapse tiny numerical noise before greedy selection so tie-breaks
            # are driven by explicit ordering rules.
            quant = float(getattr(self, "deterministic_score_quant", 128.0))
            spike_scores = torch.round(spike_scores * quant) / quant
        ...
        while torch.any(spike_scores):
            # start = perf_counter()
            if getattr(self, "deterministic_mode", 0) == 2:
                selected_candidate = self._select_deterministic_spike_candidate(spike_scores, peak_ind_flat)
                if selected_candidate is None:
                    break
                spike_seq_idx, spike_idx, seq_idx = selected_candidate
            else:
                if self.optimize_memory:
                    spike_seq_idx = torch.argmax(spike_scores)
                    spike_idx = torch.div(spike_seq_idx, self.num_seqs, rounding_mode='floor')
                    seq_idx = torch.remainder(spike_seq_idx, self.num_seqs)
                else:
            ...
            if self.optimize_memory:
                pending_seq_indices.append(seq_idx)
                pending_spike_times.append((spike_time_in_chunk + spike_times_frame_offset) / self.samp_freq)
                pending_valid_mask.append(spike_time_in_chunk >= ignore_spikes_before)
            elif spike_time_in_chunk >= ignore_spikes_before:
                spike_time = (spike_time_in_chunk + spike_times_frame_offset) / self.samp_freq
            ...
            spike_scores *= seq_no_overlap_mask[seq_idx] | (torch.abs(peak_ind_2d - offset_spike_time) > self.overlap)
            # end = perf_counter()
            # cur_delay_split_spike += ((end-start)*1000)
            # spike_detected = True
        ...
        if self.optimize_memory and pending_seq_indices:
            seq_idx_tensor = torch.stack(pending_seq_indices)
            spike_time_tensor = torch.stack(pending_spike_times)
            valid_mask = torch.stack(pending_valid_mask)
            detections = list(zip(
                seq_idx_tensor[valid_mask].to("cpu").tolist(),
                spike_time_tensor[valid_mask].to("cpu").tolist(),
            ))
        return detections
    ...
    def calc_pre_medians(self, pre_median_frames: torch.Tensor):
        ...
        pre_medians = torch.median(torch.abs(pre_median_frames[self.comp_elecs_flattened]).cpu(), dim=1).values.to(self.device)
    ...
    def set_model(self, model, num_elecs=None, input_size=None, model_inter_path=None):
        ...
        if model_inter_path is not None:
            self.model_inter_path = Path(model_inter_path)
        if not hasattr(self, "model_inter_path"):
            self.model_inter_path = None
        if getattr(self, "deterministic_mode", 0) == 2:
            self.model = model.model.conv.to(device=self.device, dtype=self.dtype)
            self.model.eval()
            self.input_size = input_size
            return
        compiled_model_path = (
            self.model_inter_path / ModelSpikeSorter.compiled_name
            if self.model_inter_path is not None
            else None
        )
        use_cached = (
            compiled_model_path is not None
            and compiled_model_path.exists()
            and ModelSpikeSorter.can_load_compiled()
        ...
        if use_cached:
            try:
                loaded_model = ModelSpikeSorter.load_compiled(self.model_inter_path, device=self.device)
                try:
                    loaded_device = next(loaded_model.parameters()).device
                except StopIteration:
                    loaded_device = torch.device(self.device)
                target_device = torch.device(self.device)
                devices_match = loaded_device.type == target_device.type
                if devices_match and loaded_device.type == "cuda":
                    loaded_idx = torch.cuda.current_device() if loaded_device.index is None else loaded_device.index
                    target_idx = torch.cuda.current_device() if target_device.index is None else target_device.index
                    devices_match = loaded_idx == target_idx
                if not devices_match:
                    raise RuntimeError(
                        f"Cached model is on {loaded_device}, expected {self.device}"
                    )
                self.model = loaded_model
            except Exception:
                self.model = model.compile(
                    num_elecs,
                    model_save_path=self.model_inter_path,
...
def _open_npy_memmap(save_path, shape, dtype):
    array = np.lib.format.open_memmap(save_path, mode="w+", dtype=dtype, shape=shape)
    array.flush()
    return array
...
def save_traces(recording, inter_path,
                ...
                verbose=True, optimize_memory=False):
    """
    ...
    if isinstance(recording, MaxwellRecordingExtractor):
        ...
        save_traces_mea(recording._kwargs['file_path'], scaled_traces_path, 
                        ...
                        verbose=verbose, optimize_memory=optimize_memory)
    else:
...
def save_traces_si(recording: BaseRecording, scaled_traces_path,
    ...
    if optimize_memory:
        traces = _open_npy_memmap(scaled_traces_path, (num_elecs, end_frame-start_frame), dtype)
        del traces
    else:
...
def save_traces_mea(rec_path, save_path,
    ...
    if optimize_memory:
        traces = _open_npy_memmap(save_path, (len(chan_ind), end_frame-start_frame), dtype)
        del traces
    else:
...
def run_detection_model(recording,
                        ...
                        device="cuda", verbose=True, optimize_memory=False, deterministic_mode=0):
    """
    ...
    if deterministic_mode == 2:
        # Keep inference on one deterministic PyTorch path.
        model_compiled = model.model.conv.to(device=device, dtype=torch.float16)
        model_compiled.eval()
    else:
    ...
    if optimize_memory:
        traces_all = _open_npy_memmap(model_traces_path, scaled_traces.shape, np_dtype)
        outputs_all = _open_npy_memmap(
            model_outputs_path,
    ...
    traces_runtime_buffer = None
    traces_cpu_buffer = None
    outputs_cpu_buffer = None
    ...
    with torch.no_grad():
        for start_frame in tqdm(all_start_frames):
            if optimize_memory:
                traces_torch = _copy_array_to_reusable_torch_buffer(
                    scaled_traces[:, start_frame:start_frame+sample_size],
                    device=device,
                    dtype=torch.float16,
                    buffer=traces_runtime_buffer,
                )
                traces_runtime_buffer = traces_torch
            else:
                traces_torch = torch.tensor(scaled_traces[:, start_frame:start_frame+sample_size], device=device, dtype=torch.float16)
            traces_torch -= torch.median(traces_torch.cpu(), dim=1, keepdim=True).values.to(device)
            outputs = model_compiled(traces_torch[:, None, :] * input_scale * inference_scaling)

            if optimize_memory:
                traces_cpu_buffer = _ensure_reusable_torch_buffer(
                    traces_cpu_buffer,
                    traces_torch.shape,
                    device="cpu",
                    dtype=torch.float16,
                )
                outputs_cpu_buffer = _ensure_reusable_torch_buffer(
                    outputs_cpu_buffer,
                    outputs[:, 0, :].shape,
                    device="cpu",
                    dtype=outputs.dtype,
                )
                traces_cpu_buffer.copy_(traces_torch)
                outputs_cpu_buffer.copy_(outputs[:, 0, :])
                traces_all[:, start_frame:start_frame + sample_size] = traces_cpu_buffer.numpy()
                outputs_all[:, start_frame:start_frame + num_output_locs] = outputs_cpu_buffer.numpy()
            else:
                outputs = outputs.cpu()
    ...
    if remaining_frames > 0:
        ...
        traces_torch -= torch.median(traces_torch.cpu(), dim=1, keepdim=True).values.to(device)
        with torch.no_grad():
            outputs = model(traces_torch[:, None, :] * input_scale * inference_scaling)
        if optimize_memory:
            traces_cpu_buffer = _ensure_reusable_torch_buffer(
                traces_cpu_buffer,
                traces_torch.shape,
            ...
            outputs_cpu_buffer = _ensure_reusable_torch_buffer(
                outputs_cpu_buffer,
                outputs[:, -remaining_frames:].shape,
                device="cpu",
                dtype=outputs.dtype,
            )
            traces_cpu_buffer.copy_(traces_torch)
            outputs_cpu_buffer.copy_(outputs[:, -remaining_frames:])
            traces_all[:, -remaining_frames:] = traces_cpu_buffer.numpy()[:, -remaining_frames:]
            outputs_all[:, -remaining_frames:] = outputs_cpu_buffer.numpy()
        else:
            outputs = outputs.cpu()
...
def form_coc_clusters(root_elec, params):
    ...
    optimize_memory = params.get("optimize_memory", False)
    ...
    if optimize_memory:
        _, root_amp_medians, max_amp_medians = _compute_chunked_root_max_amp_medians(
            all_traces=all_traces,
            crossing_output_frames=crossing_output_frames,
            root_elec=root_elec,
            max_amp_elecs=max_amp_elecs,
            pre_median_frames=pre_median_frames,
            n_before=n_before,
            n_after=n_after,
        )
...
def form_all_clusters(params):
    ...
    deterministic_mode = params.get('deterministic_mode', 0)
    with Pool(processes=num_processes) as pool:
        imap_fn = pool.imap if deterministic_mode > 0 else pool.imap_unordered
        for clusters in tqdm(imap_fn(_form_all_clusters, tasks), total=len(tasks)):
...
def _compute_chunked_amp_medians(
    traces,
    output_frames,
    all_latencies,
    pre_median_frames,
    ...
    num_spikes = output_frames.size
    num_elecs = traces.shape[0]
    elec_ids = np.arange(num_elecs)

    if num_spikes == 0:
        empty = np.empty((0, num_elecs))
        return empty, empty

    pre_offsets = np.arange(-pre_median_frames - n_before, -n_before)
    pre_medians = None
    all_amp_medians = None

    for start in range(0, num_spikes, chunk_size):
        end = min(start + chunk_size, num_spikes)
        chunk_output_frames = output_frames[start:end]

        # Avoid materializing the full traces window for every spike/electrode at once.
        pre_windows = traces[
            elec_ids[:, None],
            chunk_output_frames[:, None, None] + pre_offsets,
        ]
        chunk_pre_medians = np.median(np.abs(pre_windows), axis=2)
        chunk_pre_medians = np.clip(chunk_pre_medians / 0.6745, a_min=0.5, a_max=None)

        amp_frames = chunk_output_frames[:, None] + all_latencies[start:end]
        chunk_amp_medians = (
            np.abs(traces[elec_ids[None, :], amp_frames]) / chunk_pre_medians
        )

        if pre_medians is None:
            pre_medians = np.empty((num_spikes, num_elecs), dtype=chunk_pre_medians.dtype)
            all_amp_medians = np.empty((num_spikes, num_elecs), dtype=chunk_amp_medians.dtype)

        pre_medians[start:end] = chunk_pre_medians
        all_amp_medians[start:end] = chunk_amp_medians

    return pre_medians, all_amp_medians


def _compute_chunked_root_max_amp_medians(
    all_traces,
    ...
    num_crossings = crossing_output_frames.size
    max_amp_elecs = np.asarray(max_amp_elecs, dtype=int)
    num_max_amp_elecs = max_amp_elecs.size

    if num_crossings == 0:
        empty_pre = np.empty((0, 1 + num_max_amp_elecs))
        empty = np.empty((0,))
        return empty_pre, empty, empty

    elecs = np.concatenate(([root_elec], max_amp_elecs))
    pre_offsets = np.arange(-pre_median_frames - n_before, -n_before)
    amp_offsets = np.arange(-n_before, n_after + 1)

    pre_medians = None
    root_amp_medians = None
    max_amp_medians = None

    for start in range(0, num_crossings, chunk_size):
        end = min(start + chunk_size, num_crossings)
        chunk_frames = crossing_output_frames[start:end]

        pre_windows = all_traces[
            elecs[:, None],
            chunk_frames[:, None, None] + pre_offsets,
        ]
        chunk_pre_medians = np.median(np.abs(pre_windows), axis=2)
        chunk_pre_medians = np.clip(chunk_pre_medians / 0.6745, a_min=0.5, a_max=None)

        chunk_root_amp_medians = all_traces[root_elec, chunk_frames] / chunk_pre_medians[:, 0]

        if num_max_amp_elecs == 0:
            chunk_max_amp_medians = np.zeros(end - start, dtype=chunk_root_amp_medians.dtype)
        else:
            max_amp_windows = all_traces[
                max_amp_elecs[:, None],
                chunk_frames[:, None, None] + amp_offsets,
            ]
            chunk_max_amp_medians = np.min(max_amp_windows, axis=2)
            chunk_max_amp_medians = np.max(
                np.abs(chunk_max_amp_medians) / chunk_pre_medians[:, 1:],
                axis=1,
            )

        if pre_medians is None:
            pre_medians = np.empty((num_crossings, 1 + num_max_amp_elecs), dtype=chunk_pre_medians.dtype)
            root_amp_medians = np.empty(num_crossings, dtype=chunk_root_amp_medians.dtype)
            max_amp_medians = np.empty(num_crossings, dtype=chunk_max_amp_medians.dtype)

        pre_medians[start:end] = chunk_pre_medians
        root_amp_medians[start:end] = chunk_root_amp_medians
        max_amp_medians[start:end] = chunk_max_amp_medians

    return pre_medians, root_amp_medians, max_amp_medians
...
def setup_cluster(cluster, params, n_cocs=None):
    ...
    if optimize_memory:
        _, all_amp_medians = _compute_chunked_amp_medians(
            traces=traces,
...
def intra_merge(all_clusters, params):
    ...
    with Pool(processes=num_processes) as pool:
        imap_fn = pool.imap if deterministic_mode > 0 else pool.imap_unordered
        imap = imap_fn(_intra_merge, tasks)
        if verbose:
            imap = tqdm(imap, total=len(tasks))
...
def order_sequences(sequences):
    # Canonical sequence ordering for stable sequence indices across runs.
    def _seq_sort_key(seq):
        first_spike = seq.spike_train[0] if len(seq.spike_train) > 0 else 0
        spike_count = len(seq.spike_train)
        root_amp = round(float(getattr(seq, "root_amp_median", 0.0)), 6)
        split_elecs = tuple(sorted(getattr(seq, "split_elecs", ())))
        return (int(seq.root_elec), float(first_spike), -int(spike_count), root_amp, split_elecs)
    ordered = sorted(sequences, key=_seq_sort_key)
