...
from torchbci.rtsort import utils, plot
...
class ModelSpikeSorter(nn.Module):
    ...
    def compile(self, n_dim_0: int, model_save_path=None,
        ...
        model_traced = torch.jit.trace(model, [torch.rand(
            n_dim_0, self.num_channels_in, input_size, dtype=dtype, device=device)])
        if not hasattr(model_traced, "_c"):
            print("Skipping TensorRT compilation because torch.jit.trace did not return a TorchScript module. This can happen under profilers such as Scalene.")
            return model
        if not TENSORRT:
            ...
            if model_save_path is not None:
                torch.jit.save(model_traced, Path(model_save_path) / ModelSpikeSorter.compiled_name)
            return model_traced
        
        try:
            model_compiled = torch_tensorrt.compile(model_traced,
            ...
            if "_c" not in str(exc):
                raise
            print("Skipping TensorRT compilation because the current runtime did not preserve TorchScript internals needed by torch_tensorrt.")
            return model

        if model_save_path is not None:
            torch.jit.save(model_compiled, model_save_path /
                           ModelSpikeSorter.compiled_name)
        return model_compiled

    @staticmethod
    def load_compiled(model_save_path, device="cpu"):
        """
        Load saved compiled model
        """
        return torch.jit.load(Path(model_save_path) / ModelSpikeSorter.compiled_name, map_location=device)

    @staticmethod
    def can_load_compiled():
        """
        Scalene can patch TorchScript classes so serialized ScriptModules cannot be reconstructed.
        """
        return hasattr(torch.jit.RecursiveScriptModule, "_construct")
